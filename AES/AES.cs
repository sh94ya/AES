    ...
    public class AES
    {
            ...
            if (Nk < 6)
            {
                ...
                for (int j = Nk; j < 2 * Nb * (Nk + 1) + 4; j += Nk)
                {
                    for (int k = 0; k < 4; k++)
                    {
                        ...
                        W[k, j] = Convert.ToByte((W[k, j - Nk]) ^ (temp[k]) ^ (Rcon[j / Nk - 1][k]));
                    }
                    for (int i = 1; i < Nk && i + j < Nb * (Nr + 1); i++)
                        for (int k = 0; k < 4; k++)
                            W[k, i + j] = Convert.ToByte((W[k, i + j - Nk]) ^ (W[k, i + j - 1]));
                }
            }
            //Ключ 192
               ...
               for (int j = Nk; j < 2 * Nb * (Nk + 1); j += Nk)
               {
                   ...
                   for (int k = 0; k < 4; k++)
                   {
                       if (j + 4 < W.Length/4)
                       {
                           ...
                           for(int l=0;l<4;l++)
                               temp1[l] = W[l,j+3];
                           byte[] temp = SubWord(temp1);
                           if(j+4<W.Length/4)
                           W[k, j + 4] = Convert.ToByte(W[k, j + 4 - Nk] ^ temp[k]);
                       }
                   }
                   for (int i = 5; i < Nk; i++)
                       for (int k = 0; k < 4; k++)
        ...
        public void AddRoundKey(int index)
        {
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    state[j, i] = Convert.ToByte(Convert.ToInt32(state[j, i]) ^ Convert.ToInt32(W[j, (index * Nb) + i]));
                }
            }
        }
        #endregion
